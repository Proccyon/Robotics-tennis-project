...
#----------Summary----------#
#This is the main environment file. The environment is implemented in the Game
#class. If you just want to run the environment create a game instance and
#run game.run(Rounds, draw, blueAgent, redAgent), see the bottom of the file. 
#You can run this file to run 20 rounds with random agents.
#---------------------------#
...
class TennisRobot():
    ...
    def __init__(self, game, team, groundPosition, groundPositionFake, platformRotation, rotatorMaxTheta, rotatorMaxVelocity, maxMoverVelocity):
        ...
        #The mover at the top doesnt actually do anything except block balls
...
class Game:
    ...
    def step(self):
        ...
        #Used to measure environment speed versus agent speed
        if(endPhys - endAnn > 0):
    ...
    #We slow the game down in certain situations to ensure stable physics.
    #This finds that slowing ratio
    ...
    #Does all the physics for dt seconds(robot movement, ball movement, collisions)
    def physicsStep(self, dt):
        
        allowedTime = dt
        
        #We keep looping to see if the ball interacts with stuff untill time runs out
        while(allowedTime > 0):
            
            #If a robot took too long then award points and reset
            ...
            #If ball is out of bounds somehow(yes this can happen) then reset
            ...
            #Loop through all borders to see if ball with collide with them
            for border in self.borderList:
                ...
                #If border too far away dont bother doing any calculations
                if(travelDistance + self.ball.radius < border.distanceTo(self.ball.position)):
                    continue
                
                #Calculate where and when ball will hit border under current circumstances
                collisionTime, collisionSuccess, collisionArgs = border.getCollision(self.ball)
                
                #If ball will hit border before end of tick, add it to the list
            ...
            if(len(borders) > 0):
                ...
                #Grab the border which we are set to hit first
                ...
                #Keep track of who touched to ball last to know who to blame
                ...
                #If border belonged to robot, award points and reset
            ...
            #In case no border is bound to be hit, just move ball and robot along
            else:
    ...
    def reset(self):
        ...
        if(self.turn == "blue"):
            ballAngle = np.random.uniform(-0.30 * np.pi, 0.30 * np.pi)
            self.turn = "red"
        else:
            ballAngle = np.random.uniform(0.7 * np.pi, 1.3 * np.pi)
            self.turn = "blue"
...
if __name__ == "__main__":
    ...
    rounds = 20
    game.run(rounds, True, blueAgent = blueAgent, redAgent = redAgent)
